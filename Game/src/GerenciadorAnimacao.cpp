...
void GR::GerenciadorAnimacao::animarEfeitoJogador(){
    animaEfeito.anima(jogador->getArea()->getArea(), false);
}
...
void GR::GerenciadorAnimacao::atualizaLavas(){
    if(relogioLavas.getElapsedTime().asMilliseconds() >= 70){
...
void GR::GerenciadorAnimacao::atualizarJogador(int estado){
    ...
    switch (estado){
        case ANDANDO:
            ...
            animou = animaAndando.anima(jogador->getShape(), jogador->getDirecao(), false);
            ultimaAnimacaoJogador = ANDANDO;
            break;
        case CORRENDO:
            if(ultimaAnimacaoJogador != CORRENDO) animaCorrendo.resetaAnimacao();
            animou = animaCorrendo.anima(jogador->getShape(), jogador->getDirecao(), false);
            ultimaAnimacaoJogador = CORRENDO;
            break;
        case ATACANDO:
            if(ultimaAnimacaoJogador != ATACANDO) animaAtacando.resetaAnimacao();
            animou = animaAtacando.anima(jogador->getShape(), jogador->getDirecao(), false);
            ultimaAnimacaoJogador = ATACANDO;
            break;
        default:
            if(ultimaAnimacaoJogador != PARADO) animaParado.resetaAnimacao();
            animou = animaParado.anima(jogador->getShape(), jogador->getDirecao(), false);
            ultimaAnimacaoJogador = PARADO;
            break;
    }

    if(jogador->getEfeito()){ 
        animou = animaEfeito.anima(jogador->getArea()->getArea(), jogador->getDirecao(), false);
        if(animaEfeito.getFrameAtual() >= animaEfeito.getNumFrames() - 1){
